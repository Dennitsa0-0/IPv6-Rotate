...
Installation reads `.env` and writes `/etc/default/ipv6-rotate`.
...
| `LANGUAGE` | CLI language: `auto`, `en`, `ru` | `auto` |
...
cp -a /etc/default/ipv6-rotate .env
chmod 600 .env
bash install.sh --no-enable-timer
ipv6-rotate validate
ipv6-rotate status
...
Установка читает `.env` и пишет `/etc/default/ipv6-rotate`.
...
| `LANGUAGE` | язык CLI: `auto`, `en`, `ru` | `auto` |
